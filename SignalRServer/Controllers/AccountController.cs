    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                // register user and post that the uer has joined the game
                // info is in the view model but could be grabbed direct from info from cookie

                PostToFacebook(model.AccessToken, model.FaceBookName, model.id);
        ...
        public async void PostToFacebook(string AccessToken, string UserName, string id )
        {
            try
            {
                Dictionary<string, string> properties = new Dictionary<string, string>
            {
                ...
                { "message",  UserName + " has Joined our Test Massive online Game on http://localhost:55351/"  }
            };

                FormUrlEncodedContent authentication = new FormUrlEncodedContent(properties);
                HttpClient client = new HttpClient();
                HttpResponseMessage response = 
                    await client.PostAsync(String.Format("https://graph.facebook.com/{0}/feed", 
                    id.ToString()), authentication);
